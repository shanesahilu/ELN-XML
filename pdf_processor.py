...
def extract_excel_data_from_base64(base64_string):
    """Extracts data from a base64 encoded Excel file."""
    sheets_data = []
    try:
...
def create_pdf_table(data_list, col_widths=None, table_style_commands=None, elements_list=None, cell_style=None, header_font_name='Helvetica-Bold', cell_font_size=7):
    """Creates a ReportLab Table object and optionally adds it to an elements list."""
...
def create_transposed_pdf_table(original_pdf_data, available_width, elements_list,
                                ...
                                cell_font_size=7):
    """Creates a transposed PDF table and adds it to elements_list. Returns True if table created."""
...
def optimize_table_for_display(table_name, headers, rows, available_width, elements_list,
    ...
    """Decides whether to render a table normally or transposed based on column count and available width."""
...
def process_xml_to_pdf(xml_string_content, schema_files_config):
    ...
    if section_set_view is None:
        if not final_elements_for_pdf: 
             final_elements_for_pdf.append(Paragraph("No 'sectionSetView' content found in the XML.", style_body))
